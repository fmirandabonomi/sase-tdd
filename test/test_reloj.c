...
void test_puedePonerseEnHora(void)
{
    ...
    Reloj_getTiempo(&tiempo);
    TEST_ASSERT_EQUAL_UINT8_ARRAY_MESSAGE(tiempoSet, tiempo, TiempoBcd_NUM_DIGITOS,
                                          "El tiempo establecido debe reflejarse en el reloj");
}
...
void test_activaAlarmaAlEstablecerTiempoDeAlarma(void)
{
    ...
    TEST_ASSERT_EQUAL_UINT_MESSAGE(0, AccionAlarma_vecesDisparada(testigoAlarma),
                                   "La alarma no se dispara si no es la hora");
}
...
void test_laAlarmaSeEjecutaAlLlegarAlTiempoDeAlarma(void)
{
    ...
    avanzaUnSegundo();
    TEST_ASSERT_EQUAL_UINT_MESSAGE(1, AccionAlarma_vecesDisparada(testigoAlarma),
                                   "La alarma se dispara al llegar la hora establecida");
}
...
void test_laAlarmaDesactiadaNoSeEjecutaAlLlegarAlTiempoDeAlarma(void)
{
    ...
    avanzaUnSegundo();
    TEST_ASSERT_EQUAL_UINT_MESSAGE(0, AccionAlarma_vecesDisparada(testigoAlarma),
                                   "La alarma desactivada no se dispara al llegar la hora establecida");
}
...
void test_puedeActivarseLaAlarma(void)
{
    ...
    avanzaUnSegundo();
    TEST_ASSERT_EQUAL_UINT_MESSAGE(1, AccionAlarma_vecesDisparada(testigoAlarma),
                                   "Se puede activar la alarma inactiva");
}
...
void test_puedePosponerLaAlarma(void)
{
    ...
    Reloj_setTiempo(&tiempo2);
    avanzaUnSegundo();
    TEST_ASSERT_EQUAL_UINT_MESSAGE(3, AccionAlarma_vecesDisparada(testigoAlarma),
                                   "La alarma pospuesta vale una sola vez");
}
...
void test_aceptaTicksPorSegundoDistintosDeUno(void)
{
    ...
    Reloj_setTiempo(&tiempoInicial);
    for (int i = 0; i < (TICKS_POR_SEGUNDO - 1); ++i)
        Reloj_tick();
    Reloj_getTiempo(&tiempo);
    TEST_ASSERT_EQUAL_UINT8_ARRAY_MESSAGE(tiempoInicial, tiempo, TiempoBcd_NUM_DIGITOS,
                                          "Solo debe avanzar luego de transcurridos los ticks prescritos");
    Reloj_tick();
    Reloj_getTiempo(&tiempo);
    TEST_ASSERT_EQUAL_UINT8_ARRAY_MESSAGE(tiempoFinal, tiempo, TiempoBcd_NUM_DIGITOS,
                                          "Luego de transcurridos los ticks prescritos debe avanzar");
}
