...
#define LOG_TIEMPO(t) \
    do { \
        ...
        LOG("%02d:%02d:%02d", base[DECENA_HORA] * 10 + base[UNIDAD_HORA], \
            base[DECENA_MINUTO] * 10 + base[UNIDAD_MINUTO], base[DECENA_SEGUNDO] * 10 + base[UNIDAD_SEGUNDO]); \
    } while (0)
#else
#define LOG(s, ...)
#define LOG_TIEMPO(t)
...
static void normalizaTiempo(TiempoBcd *tiempo)
{
    ...
    while (base[DECENA_HORA] > 2 || (base[DECENA_HORA] == 2 && base[UNIDAD_HORA] > 3)) {
